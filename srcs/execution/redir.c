...
/*   Updated: 2024/10/09 10:37:32 by arakotom         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool check_error_redir_in(t_redir_in *arg_rdr)
{
	...
	while (arg_rdr)
	{
		...
		if (access(arg_rdr->file, F_OK) != 0 || access(arg_rdr->file, R_OK) != 0)
		{
...
t_bool check_error_redir_out(t_redir_out *arg_rdr)
{
	while (arg_rdr)
	{
		...
		{
			if (arg_rdr->type == REDIR_APPEND && access(arg_rdr->file, W_OK) != 0)
			{
